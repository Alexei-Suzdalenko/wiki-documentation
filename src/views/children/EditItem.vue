    ...
    <div class="card shadow mb-4">
      <div class="card-body">
        <div class="table-responsive">
            ...
            <textarea style="overflow:hidden" rows="111"
                oninput='this.style.height = "";this.style.height = this.scrollHeight + "px"'
                class="form-control form-control-sm" placeholder="Description" v-model="description"></textarea>
        </div>
      </div>
    </div>
  ...
  methods: {
     requestCurrentItem(){
       ...
       firebase.database().ref().child('users/' + stringUid + '/items/' + this.itemId).on('value', (snapshot) => {
         currentContext.currentItem = snapshot.val(); currentContext.title = snapshot.val().title;
       });
     ...
     editItem(){
       ...
       firebase.database().ref('users/' + stringUid + '/items/' + this.itemId).update({ title: currentContextData.title})
            .then(() => {
              this.requestCurrentItem();
       ...
       firebase.firestore().collection(stringUid).doc(this.itemId).update({ description: currentContextData.description })
           .then( docRef => this.requestCurrentItem() );
     }
  }
}
</script>